...
//     // struct dentry *const root_dentry = mount_nodev(type, flags, data, fill_);
...
static struct file_system_type my_vfs_type = {
    .owner = THIS_MODULE,
    .name = "myvfs",
    // .mount = my_vfs_mount,        // вызывается при подмонтировании
    // .kill_sb = my_kill_super, // вызывается при размонтировании
};
